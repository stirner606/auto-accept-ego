...
const CDP_EVAL_TIMEOUT_MS = 2000;
...
export class CDPHandler {
  ...
  async start(config: {
    ...
    this.log(
      `Scanning ports ${BASE_PORT - PORT_RANGE} to ${BASE_PORT + PORT_RANGE}...`,
    );
  ...
  private getPages(
    port: number,
  ): Promise<Array<{ id: string; webSocketDebuggerUrl: string }>> {
  ...
  private async inject(id: string, config: any): Promise<void> {
    ...
    try {
      ...
      await this.evaluate(
        id,
        `if(window.__egoStart) window.__egoStart(${JSON.stringify(config)})`,
      );
    } catch (e: any) {
    ...
    for (const [id] of this.connections) {
      try {
        const res = await this.evaluate(
          id,
          "JSON.stringify(window.__egoGetStats ? window.__egoGetStats() : {})",
        );
        if (res?.result?.value) {
