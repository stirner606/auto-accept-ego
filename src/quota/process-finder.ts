...
export class ProcessFinder {
  ...
  private async findWindowsProcess(): Promise<ProcessInfo | null> {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        const agProcess = data.find(
          (p: any) => p.CommandLine && this.isAntigravityProcess(p.CommandLine),
        );
      ...
      console.log("[ProcessFinder] Windows process search failed:", e.message);
      return null;
    }
  }
    ...
    try {
      ...
      for (const line of lines) {
        if (
          ...
          this.isAntigravityProcess(line)
        ) {
      ...
      console.log("[ProcessFinder] Unix process search failed:", e.message);
      return null;
    }
    return null;
  }
  ...
  async getListeningPorts(pid: number): Promise<number[]> {
    try {
      ...
      console.log("[ProcessFinder] Port enumeration failed:", e.message);
      return [];
    }
  }
}
